...
| `claude mcp list` reports `✓ Connected` but Claude Code shows 0 tools (server silently dropped) | `string?` / `int?` / `bool?` parameter emits `type: ["string","null"]` (union null) in tools/list; Claude Code MCP loader rejects union-null types and drops the **entire server** (not just the offending tool). Confirmed on `ModelContextProtocol` SDK 1.3.0 + Claude Code 2.1.145 (2026-05-23). | Replace `string? x = null` with `string x = ""` + a sentinel-to-null conversion at the method boundary (`string.IsNullOrEmpty(x) ? null : x`). Same for `int? → int x = -1` and `bool? → bool x = false`. Diagnose with `echo '{"jsonrpc":"2.0","id":1,"method":"initialize",...}' \| your.exe` and grep the `tools/list` response for `["string","null"]`. |
